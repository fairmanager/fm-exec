...
/**
 * Execute a command through the default shell and redirect the output to the parent process
 * @param {String} command The command to run.
 * @param {Array<String>} [args] And array of arguments to pass to the command. These arguments will be appended to
 * command. Thus, you can provide them with the command directly. The parameter is respected for compatibility with
...
function execute( command, args, options ) {
	...
	options.emit          = options.emit === true;
	...
	return new Promise( function resolver( resolve, reject ) {
		var childProcess = exec( command, options );
